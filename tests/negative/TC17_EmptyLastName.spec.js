...
test.describe('TC17 - Empty Last Name', () => {
  test('TC17 - Verify Checkout with Empty Last Name', async ({ page }) => {
    const loginPage = new LoginPage(page);
    const inventoryPage = new InventoryPage(page);
    const cartPage = new CartPage(page);
    const checkoutPage = new CheckoutPage(page);
    const { username, password } = loginData.validUsers[0];
    const { firstName, postalCode } = checkoutData.checkoutUser;
    ...
    await test.step('Enter information with empty last name', async () => {
      await checkoutPage.enterUserInformation(firstName, '', postalCode);
